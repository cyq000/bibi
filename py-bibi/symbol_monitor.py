...
# 1小时:
# 1、所有持仓用户、大户持仓、大户持仓量、三个指标 多单 都大于 60%  如果价格已经上涨并遇到阻力,可能要跌
# 2、所有持仓用户、大户持仓、多空比 大于 65%,大户持仓量多空比 小于 50% 可能要涨
# 4、所有持仓用户、大户持仓、多空比 大于 60%,大户持仓量多空比 大于 70% 如果价格长时间在低点,可能要涨
# 5、合约持仓量大于前一次的1.1倍,突然增量,有行情
...
max_24h_volume = 80000000  # 24 小时最大成交额 5000 万 USDT
time_interval = 3600  #900：15分钟 #3600：1小时 每小时获取一次数据
num_threads = 4  # 线程数

time_printf = 10800 # 打印过的币种在 3小时内 不再打印

# 获取 24 小时成交额在 500 万 USDT 至 5000 万 USDT 之间的币种
def get_symbols():
    symbols = set()
...
# 检查主动买入/卖出量是否激增
def check_aggressive_volume_spike(symbol, period="15m", limit=4):
    period = "15m" # 不管外部传入多少，都使用6小时，因为发现6小时来判断比较好
    ...
    buy_sell_ratio = [float(item['buySellRatio']) for item in taker_vol_data]

    # 最新值
    buy_latest = buy_vols[-1]
    sell_latest = sell_vols[-1]
    ratio_latest = buy_sell_ratio[-1]
    ...
    # 是否激增（15倍）
    multiple = 15
    bool_buy_spike = buy_latest > buy_avg_prev * multiple
    bool_sell_spike = sell_latest > sell_avg_prev * multiple

    # 倍数
    buy_multiple = buy_latest / buy_avg_prev if buy_avg_prev > 0 else float('inf')
    sell_multiple = sell_latest / sell_avg_prev if sell_avg_prev > 0 else float('inf')
    
    # if buy_spike or sell_spike:
    # print(f"⚠️ {symbol} 主动买/卖激增！买入 {buy_multiple:.2f} 倍，卖出 {sell_multiple:.2f} 倍,买入卖出比 {ratio_latest:.2f}")
            

    return bool_buy_spike, bool_sell_spike, buy_multiple, sell_multiple, ratio_latest
...
def check_price_spike(symbol_pair, contract_type="PERPETUAL", interval="15m", limit=5):
...
def build_symbol_metrics_lines(
    ...
    increase: float = None,
    increase_text: str = "15分钟K线当前收盘价较前4条收盘价最高值涨幅"
) -> list[str]:
    lines = []
...
def get_long_short_ratios(symbol):
    try:
        ...
        open_interest_url = f"https://fapi.binance.com/futures/data/openInterestHist?symbol={symbol}&period={period_time}&limit=4"
        open_interest_data = exchange.fetch(open_interest_url)
        if not open_interest_data or len(open_interest_data) < 4:
            return False
        ...
        open_interest_avg_prev3 = sum(open_interest_volumes[:-1]) / 3     # 前三次平均
        open_interest_increase = open_interest_latest > open_interest_avg_prev3 * 1.2  # 是否激增
        open_interest_multiple = open_interest_latest / open_interest_avg_prev3 # 倍数
        ...
        # 检查主动买入/卖出量是否激增
        buy_spike, sell_spike, buy_mult, sell_mult, buy_sell_ratio = check_aggressive_volume_spike(symbol, period_time, 4)
        # if buy_spike or sell_spike:
        #     print(f"⚠️ {symbol} 主动买/卖激增！买入 {buy_mult:.2f} 倍，卖出 {sell_mult:.2f} 倍,买入卖出比 {buy_sell_ratio:.2f}")
        
        
        # 检查当前 K 线收盘价是否较前 4 条 K 线中任意一条上涨超过 7%
        bool_increase, increase = check_price_spike(symbol)
        ...
        #         'long_account': global_long_account,
        #         'short_account': global_short_account,
        ...
        #     '币种': symbol,
        #     '信息': {
        #         '<所有用户><多空人数持仓比> ': global_long_account,
        #         '<持仓大户><多空人数持仓比> ': top_account_long_account,
        #         '<持仓大户><持仓量多空比>   ': top_position_long_account,
        #         'timestamp': global_timestamp
        #         },
        #     '注意':"<1小时级别> 三个指标 多单 都大于 60%  可能要跌"
        #     }
        ...
        lines7 = [
            ...
            f"⚠️ 获取最近持仓量数据,本次持仓量大于前三次平均的**{open_interest_multiple}**倍 ",
            f"💰 RSI: **{rsi_value}**",
            f"⚠️ 15分钟的的k线.合约持仓量.主动买/卖激增！买入 {buy_mult:.2f} 倍，卖出 {sell_mult:.2f} 倍,买入卖出比 {buy_sell_ratio:.2f}"
        ]
        ...
        # # 最近持仓量数据，并判断是否激增（本次大于前三次平均的1.2倍）
        ...
        if bool_increase :
            ...
            lines = build_symbol_metrics_lines(
                ...
                increase = increase,
                increase_text = "15分钟K线当前收盘价较前4条收盘价最高值涨幅"
            )
