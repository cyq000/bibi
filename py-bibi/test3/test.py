...
# 参数设置
min_24h_volume = 1000000  # 24 小时最小成交额 100 万 USDT
...
def build_symbol_metrics_lines(
    ...
    buy_mult_2: float = None,
    sell_mult_2: float = None,
    buy_mult_12: float = None,
    sell_mult_12: float = None,
) -> list[str]:
    lines = []
    ...
    if open_interest_multiple is not None:
        lines.append(f"⚠️ 2小时的持仓量大于前三次平均的**{open_interest_multiple}**倍 ")
    if buy_mult_2 is not None and sell_mult_2 is not None:  
        lines.append(f"⚠️ 2小时的合约主动买/卖持仓量激增！买入 {buy_mult_2:.2f} 倍，卖出 {sell_mult_2:.2f} 倍")       
    if buy_mult_12 is not None and sell_mult_12 is not None:  
        lines.append(f"⚠️ 12小时的合约主动买/卖持仓量激增！买入 {buy_mult_12:.2f} 倍，卖出 {sell_mult_12:.2f} 倍")      
    return lines
...
def get_long_short_ratios(symbol):
    try:
        ...
        multiple_12 = 4
        limit = 4
        buy_spike_2, sell_spike_2, buy_mult_2, sell_mult_2, buy_sell_ratio_2 = check_aggressive_volume_spike(symbol, period_time_2,multiple_2, limit)
        # if buy_spike_2 or sell_spike_2:
        #     print(f"⚠️ {symbol} 主动买/卖激增！买入 {buy_mult_2:.2f} 倍，卖出 {sell_mult_2:.2f} 倍,买入卖出比 {buy_sell_ratio_2:.2f}")
        buy_spike_12, sell_spike_12, buy_mult_12, sell_mult_12, buy_sell_ratio_12 = check_aggressive_volume_spike(symbol, period_time_12,multiple_12, limit)
        
        
        # 检查当前 K 线收盘价是否较前 4 条 K 线中任意一条上涨超过 7%
        bool_increase, increase = check_price_spike(symbol,"PERPETUAL", "2h", 5)
        ...
        if buy_spike_2 or sell_spike_2 :
            ...
            # logging.info(content) # print(f"{symbol} 符合条件3: prinf_info:{prinf_info3}")    
            ret = True
        if buy_spike_12 or sell_spike_12 :
            # content = build_feishu_post_message(title, lines7)
            # send_feishu_message(
            #     webhook_url=webhook,
            #     msg_type="post",
        ...
        if bool_increase :
            ...
            #     buy_mult_2 = buy_mult_2,
            #     sell_mult_2 = sell_mult_2
            # )
        # logging.info(f"2 symbol:{symbol},ret:{ret}")   
        if ret:
            lines = build_symbol_metrics_lines(
                symbol = symbol,
                quoteVolume = quoteVolume,
                ...
                buy_mult_2 = buy_mult_2,
                sell_mult_2 = sell_mult_2,
                buy_mult_12 = buy_mult_12,
                sell_mult_12 = sell_mult_12,
                )
...
def main():
    ...
    logging.info(f"符合条件的高流动性币种（200 万美元 < 24 小时成交额 < 8000 万美元）：{len(symbols)} 个")
